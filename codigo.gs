...
// =========================================================
function executarFechamentoAutomatico() {
  try {
    // 1. Busca os dados de hoje da base
    var dadosHoje = carregarDadosNuvem(); // Usa sua função existente
    var pedidosHoje = JSON.parse(dadosHoje);
    
    // Filtra apenas o que é de hoje
    var vendasHoje = pedidosHoje.filter(function(v) { 
      return v.timestamp && ehHoje(v.timestamp); 
    });

    if (vendasHoje.length === 0) return; // Se não tem venda, não faz nada

    // 2. Calcula os totais (mesma lógica do seu front-end)
    var totDin = 0, totPix = 0, totCred = 0, totDeb = 0, totVR = 0, totTotal = 0, qtdTapiocas = 0;
    
    vendasHoje.forEach(function(v) {
      totTotal += v.total;
      v.itens.forEach(function(i) { if(i.tipo === 'tapioca' || i.tipo === 'extra') qtdTapiocas += i.quantidade; });
      var arrPag = v.pagamentosMistos || [{forma: v.formaPagamento, valor: v.total}];
      arrPag.forEach(function(pag) {
        if(pag.forma === 'Dinheiro') totDin += pag.valor;
        if(pag.forma === 'PIX') totPix += pag.valor;
        if(pag.forma === 'Cartão de Crédito') totCred += pag.valor;
        if(pag.forma === 'Cartão de Débito') totDeb += pag.valor;
        if(pag.forma === 'VR') totVR += pag.valor;
      });
    });

    // 3. Monta o objeto de resumo
    var resumo = {
      data: new Date().toLocaleDateString('pt-BR'),
      total: formatarMoeda(totTotal),
      dinheiro: formatarMoeda(totDin),
      pix: formatarMoeda(totPix),
      credito: formatarMoeda(totCred),
      debito: formatarMoeda(totDeb),
      vr: formatarMoeda(totVR),
      qtdTapiocas: qtdTapiocas
    };

    // 4. Salva na planilha (chama sua função existente)
    salvarFechamentoDiaPlanilha(JSON.stringify(resumo));
    
    // 5. Limpa a nuvem
    salvarNuvemCompleta("[]");
    
    Logger.log("Fechamento automático concluído com sucesso!");
    
  } catch (e) {
    Logger.log("Erro no fechamento automático: " + e.toString());
  }
}
